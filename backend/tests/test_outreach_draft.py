...
def _seed_revealed_contact(suffix: str = "1") -> tuple[str, str]:
    db = SessionLocal()
    try:
...
def test_outreach_draft_success_traced(client: TestClient, monkeypatch) -> None:
    contact_id, email = _seed_revealed_contact("ok")
    ...
    def _fake_generate(db, cid):
        assert cid == contact_id
        return fake, email

    with patch("app.services.outreach_draft.generate_outreach_draft", side_effect=_fake_generate):
        # Prefer patching at the router import path
        pass
...
def test_outreach_draft_calls_llm_with_prompt_meta(client: TestClient) -> None:
    contact_id, email = _seed_revealed_contact("llm")
