...
from app.services.resume.tournament import (
    AlignmentEvidence, argument_grounded, majority_from_votes, maybe_run_tournament,
    pairwise_cache_key, select_pairs_by_gap, agreement_label,
)

def _job() -> Job:
    return Job(id="j1", source="t", source_job_id="s1", title="ML Engineer", company="Co",
               location="Remote", description="Need PyTorch and MLflow.", apply_url="https://ex.com/j", skills=["PyTorch"])

def _ev(rid: str, cov: float, ch: str | None = None, rows: list | None = None) -> AlignmentEvidence:
    return AlignmentEvidence(
        resume_id=rid, content_hash=ch or rid, coverage=cov, top_units=[f"evidence for {rid}"],
        alignment_rows=rows or [{"kind": "must", "requirement": "PyTorch", "evidence_unit": f"built {rid} with PyTorch", "status": "hit", "strength": "strong"}],
        filename=f"{rid}.pdf",
    )
    ...
    with patch("app.services.inference.llm.httpx.Client", FakeClient):
        with patch("app.services.inference.llm.observability.traced_call") as tc:
            class Ctx:
                ...
                def __exit__(self, *a): return False
            def _traced(*a, **kw):
                captured["trace_model"] = kw.get("model"); return Ctx()
...
def test_panel_majority_and_independent_flip(monkeypatch) -> None:
    ...
    def fake_complete_json(prompt, schema, **kwargs):
        ...
        a_is_left = f"Resume A (a.pdf)" in prompt or "Resume A (a.pdf)" in prompt
        ...
        return schema(winner=winner, margin="decisive", key_differences=["clearer PyTorch production evidence"], reason="decisive unit on must-have PyTorch evidence")
    with patch("app.services.resume.tournament.llm.complete_json", side_effect=fake_complete_json):
        with patch("app.services.resume.tournament.load_prompt") as lp:
            lp.return_value = MagicMock(body="Judge which resume is better.", system="json", version="1", content_hash="ph", name="pairwise_judge", model_params={})
...
def test_panel_max_pairs_cap(monkeypatch) -> None:
    ...
    def fake_complete_json(prompt, schema, **kwargs):
        calls["n"] += 1
        return schema(winner="A", margin="decisive", key_differences=["clearer PyTorch production evidence"], reason="decisive unit on must-have PyTorch evidence")
    with patch("app.services.resume.tournament.llm.complete_json", side_effect=fake_complete_json):
        with patch("app.services.resume.tournament.load_prompt") as lp:
            lp.return_value = MagicMock(body="Judge", system="json", version="1", content_hash="ph", name="pairwise_judge", model_params={})
            result = maybe_run_tournament(_job(), reqs, ordered, use_llm=True, db=None)
...
def test_flag_off_single_judge_one_call(monkeypatch) -> None:
    ...
    calls = {"n": 0, "models": []}
    def fake_complete_json(prompt, schema, **kwargs):
        calls["n"] += 1; calls["models"].append(kwargs.get("llm_model"))
        return schema(winner="A", margin="decisive", key_differences=["clearer PyTorch production evidence"], reason="decisive unit on must-have PyTorch evidence")
    with patch("app.services.resume.tournament.llm.complete_json", side_effect=fake_complete_json):
        with patch("app.services.resume.tournament.load_prompt") as lp:
            lp.return_value = MagicMock(body="Judge", system="json", version="1", content_hash="ph", name="pairwise_judge", model_params={})
