...
def test_greenhouse_adapter_and_html_unescape(monkeypatch):
    ...
    assert any("Stripe" in j.description or "Who we are" in j.description or "financial" in j.description.lower() for j in result)
...
def test_registry_isolation_one_source_raises(monkeypatch):
    ...
    class Ok:
        ...
        def is_enabled_for(self, c): return True
        def fetch(self, c, db=None):
            return [Job(
                id="1", source="ok_src", source_job_id="1", source_quality="feed",
                title="Python Engineer", company="Co", location="Remote",
                description="Python work", apply_url="https://example.com/1",
...
def test_dedup_prefers_direct_ats_over_aggregator():
    ...
    ats = Job(
        id="ats", source="greenhouse", source_job_id="g1", source_quality="direct_ats",
        title="Backend Engineer", company="Acme", location="SF",
        description="Python", apply_url="https://boards.greenhouse.io/x", posted_at=posted,
    )
    agg = Job(
        id="agg", source="jsearch", source_job_id="j1", source_quality="aggregator",
        title="Backend Engineer", company="Acme", location="SF",
        description="Python", apply_url="https://example.com/j", posted_at=posted - timedelta(days=2),
    )
...
def test_title_filter_keeps_swe_family_roles():
    ...
    def mk(title, desc="Build platforms for customers.", loc="Remote"):
        return Job(
            id=title[:40], source="greenhouse", source_job_id=title[:40], source_quality="direct_ats",
            title=title, company="X", location=loc, description=desc,
...
def test_post_fetch_hard_remote_and_recency():
    now = datetime.now(UTC)
    crit = _criteria(params=SearchParams(remote_mode="remote", remote_mode_pref="hard", date_window="week", use_expand=False))
    remote = Job(
        id="1", source="gh", source_job_id="1", source_quality="direct_ats", title="Backend Engineer",
        company="X", location="Remote", description="Systems", apply_url="https://x.com",
        posted_at=now, remote_mode="remote",
    )
...
def test_location_filter_us_profile_drops_japan():
    ...
    jp = Job(
        id="1", source="gh", source_job_id="1", source_quality="direct_ats", title="Backend Engineer",
        company="X", location="Tokyo, Japan", description="Systems", apply_url="https://x.com", posted_at=now,
    )
...
def test_greenhouse_skips_bad_item_not_whole_board(monkeypatch):
    payload = {
        "jobs": [
            "not-a-dict",
            {
                "id": 1, "title": "Backend Engineer", "absolute_url": "https://example.com/1",
                "location": {"name": "Remote"}, "content": "Python systems",
...
def test_merge_fetch_uses_registry(monkeypatch):
    ...
    posted = datetime.now(UTC) - timedelta(days=1)
    board_job = Job(
        id="b1", source="greenhouse", source_job_id="g1", source_quality="direct_ats",
        title="Software Engineer", company="Stripe", location="Remote",
        description="Python backend engineer role", apply_url="https://example.com/g",
        posted_at=posted, skills=["Python"],
    )
    jsearch_job = Job(
        id="j1", source="jsearch", source_job_id="js1", source_quality="aggregator",
        title="Software Engineer", company="Acme", location="Remote",
        description="Python services", apply_url="https://example.com/j",
        posted_at=posted, skills=["Python"],
    )
    db = MagicMock()
...
def test_token_boundary_no_ml_in_html():
    ...
    job = Job(
        id="1", source="gh", source_job_id="1", source_quality="direct_ats",
        title="HTML Email Designer", company="X", location="Remote",
        description="Design newsletters", apply_url="https://x.com", posted_at=now,
    )
