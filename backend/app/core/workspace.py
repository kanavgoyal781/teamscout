...
from app.db.models import (
    Contact, DriveSyncedFile, DriveSyncState, Feedback, IntentSearch, JobCache,
    JobTeamSearch, Resume, ResumeUnit, Search, TeamExtractionRecord, Workspace,
)
...
def sweep_expired_workspaces(*, now: datetime | None = None) -> int:
    ...
    try:
        ...
        for ws in expired:
            wid = ws.id
            file_paths = [
                r[0] for r in db.query(Resume.file_path).filter(Resume.workspace_id == wid).all() if r[0]
            ]
            for model in (
                ResumeUnit, Contact, TeamExtractionRecord, JobTeamSearch, Search,
                IntentSearch, Feedback, JobCache, DriveSyncedFile, DriveSyncState, Resume,
            ):
    ...
    async def __call__(self, scope: dict[str, Any], receive: Any, send: Any) -> None:
        ...
        async def send_wrapper(message: dict[str, Any]) -> None:
            ...
            if message["type"] == "http.response.start":
                ...
                raw_headers = [
                    (n, v) for n, v in raw_headers if n.decode("latin-1").lower() != "set-cookie"
                    or COOKIE_NAME.encode("latin-1") not in v
                ]
