...
async def upload_resume(request: Request, file: UploadFile = File(...), db: Session = Depends(get_db)) -> ResumeUploadResponse:
    ...
    if existing is not None:
        ...
        return ResumeUploadResponse(id=existing.id, filename=existing.filename, content_hash=existing.content_hash, confirmed=existing.confirmed, profile=stored_profile)
    destination = workspace_upload_path(wid, file_hash, file.filename)
    destination.write_bytes(data)
    row = Resume(workspace_id=wid, filename=file.filename, content_hash=file_hash, file_path=str(destination), parsed_json=profile.model_dump_json(), confirmed=False)
    db.add(row)
    db.commit()
    db.refresh(row)
    return ResumeUploadResponse(id=row.id, filename=row.filename, content_hash=row.content_hash, confirmed=row.confirmed, profile=profile)
@router.put("/{resume_id}/confirm", response_model=ResumeConfirmResponse)
def confirm_resume(resume_id: str, payload: ResumeConfirmRequest, db: Session = Depends(get_db)) -> ResumeConfirmResponse:
    wid = require_workspace_id()
