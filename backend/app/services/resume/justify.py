...
_RANK_SUPERLATIVE = re.compile(
    ...
    r"clearest\s+best|unambiguously\s+best)\b)",
    re.IGNORECASE,
)
# Inference-excuse patterns: forbid soft-pedaling missing skills via seniority/caliber.
# Kept tight to avoid false positives on legitimate "standard for this role's stack" without excuse intent.
_INFERENCE_EXCUSE = re.compile(
    r"(?i)\b(?:"
    r"standard\s+(?:libraries?|tools?|skills?|stack)?\s*(?:for|of)\b|"
    r"of\s+this\s+caliber\b|"
    r"would\s+typically\b|"
    r"presumably\b|"
    r"as\s+expected\s+for\s+(?:a\s+)?(?:senior|staff|principal|lead)\b|"
    r"implicitly\s+(?:has|knows|possesses)\b|"
    r"can\s+be\s+assumed\b|"
    r"goes\s+without\s+saying\b"
    r")",
)


def rationale_has_inference_excuse(rationale: str) -> bool:
    """True if rationale excuses missing skills via seniority/caliber/assumption language."""
...
def fit_scores_rank_monotonic(items: list[ResumeRerankItem], ranks: dict[str, int]) -> bool:
    ...
    return all(
        float(cur.fit_score) <= float(prev.fit_score) + 1e-9 for prev, cur in zip(ordered, ordered[1:], strict=False)
    )
