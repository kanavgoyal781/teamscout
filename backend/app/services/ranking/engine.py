...
def _call_rerank_llm(
    profile: ResumeProfile, alias_jobs: list[tuple[str, Job]], *, max_retries: int = 2
) -> _RerankResponse:
...
def _resolve_listwise_aliases(ordered: list[tuple[str, str]], alias_to_real: dict[str, str]) -> list[tuple[str, str]]:
    ...
    *,
    ...
    scored = hybrid_rank(
        ...
        experience_fn=lambda r: experience_fit_score(
            ...
            title=jobs_by_id[r.id].title,
            description=jobs_by_id[r.id].description,
        ),
        requirements_fn=lambda r: requirements_met_score(
            profile_skills=profile.skills,
            profile_text=profile_text,
            job_skills=jobs_by_id[r.id].skills,
            job_description=jobs_by_id[r.id].description,
        ),
        ...
        ranked.append(
            RankedJob(
                ...
                score_breakdown=ScoreBreakdown(
                    ...
                    cross_encoder=item.cross_encoder,
