...
def tokenize_tech(text: str) -> list[str]:
    if not text:
        return []
    return re.findall(r"c\+\+|c#|node\.js|react\.js|[a-z0-9]+(?:\.[a-z0-9]+)*|\bc\b|\br\b", text.lower())
...
def _phrase_level_in_tokens(needle: list[str], hay: list[str]) -> str | None:
    ...
    for i in range(len(hay) - n + 1):
        window = hay[i : i + n]
        if all(a == b for a, b in zip(needle, window, strict=True)):
            return "exact"
        if all(tokens_match(a, b) for a, b in zip(needle, window, strict=True)):
            best = "alias"
    return best
...
def align_resume(
    ...
    *,
    ...
    evidence_floor: float = DEFAULT_EVIDENCE_FLOOR,
    ...
    for i, (req_emb, req_text, weight) in enumerate(zip(requirement_embs, requirement_texts, weights, strict=True)):
        ...
        best_i, semantic = maxsim_best_unit_index(
            ...
            unit_texts=unit_texts,
            unit_sections=unit_sections,
            skills=None if is_skill else skills,
            skill_bonus=0.0 if is_skill else skill_bonus,
        )
        unit_text = unit_texts[best_i] if best_i is not None else None
        if is_skill:
            raw = skill_requirement_score(req_text, skills=skills, unit_texts=unit_texts, semantic_score=semantic)
        ...
        rows.append(
            {
                ...
                "raw_evidence_score": round(float(raw), 4),
                "status": status,
                "category": category,
