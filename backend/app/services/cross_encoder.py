...
def cross_encode(query: str, documents: list[str]) -> list[float]:
    ...
    with observability.traced_call(
        "cross_encode", model=model, check_llm_ceiling=True,
        ...
        trace.input_tokens = int(usage["total_tokens"]) if isinstance(usage, dict) and usage.get("total_tokens") is not None else est
