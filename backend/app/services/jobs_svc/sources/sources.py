...
def _job(**kw) -> Job:
    ...
    job = annotate_job(
        job, is_remote_flag=is_remote, structured_employment=employment, structured_salary_min=salary_min
    )
    ...
    def fetch(self, criteria: FetchCriteria, db: Session | None = None) -> list[Job]:
        ...
        for i, item in enumerate(raw):
            if not isinstance(item, dict):
                _skip("jsearch", "-", i, "not_object", item)
                continue
            ...
            if not title or not desc or not apply_url:
                _skip("jsearch", "-", i, "missing_required", item)
                continue
            sid = str(item.get("job_id") or item.get("job_google_link") or apply_url)
            loc = ", ".join(
                str(p).strip() for p in (item.get("job_city"), item.get("job_state"), item.get("job_country")) if p
            )
            ...
            out.append(
                _job(
                    ...
                    skills=skills,
                    is_remote=is_remote,
                    employment=item.get("job_employment_type"),
                    salary_min=sal,
                )
            )
        return out
...
class GreenhouseSource(_AtsBoardSource):
    ...
    def _parse_payload(self, payload: object, slug: str, profile: ResumeProfile) -> list[Job]:
        ...
        for i, item in enumerate(payload["jobs"]):
            if not isinstance(item, dict):
                _skip("greenhouse", slug, i, "not_object", item)
                continue
            title, apply_url, jid = (
                str(item.get("title") or "").strip(),
                str(item.get("absolute_url") or "").strip(),
                item.get("id"),
            )
            if not title or not apply_url or jid is None:
                _skip("greenhouse", slug, i, "missing_required", item)
                continue
...
class AshbySource(_AtsBoardSource):
    ...
    def _parse_payload(self, payload: object, slug: str, profile: ResumeProfile) -> list[Job]:
        ...
        for i, item in enumerate(payload["jobs"]):
            if not isinstance(item, dict):
                _skip("ashby", slug, i, "not_object", item)
                continue
            title = str(item.get("title") or "").strip()
            apply_url = str(item.get("jobUrl") or item.get("applyUrl") or "").strip()
            jid = item.get("id")
            if not title or not apply_url or not jid:
                _skip("ashby", slug, i, "missing_required", item)
                continue
            desc = str(item.get("descriptionPlain") or "").strip() or strip_html(
                str(item.get("descriptionHtml") or title)
            )
...
class RemotiveSource:
    ...
    def is_enabled_for(self, criteria: FetchCriteria) -> bool:
        return bool(
            settings.JOBS_EXTRA_SOURCES_ENABLED
            and settings.JOBS_SOURCE_REMOTIVE_ENABLED
...
class RemoteOKSource:
    ...
    def is_enabled_for(self, criteria: FetchCriteria) -> bool:
        return bool(
            settings.JOBS_EXTRA_SOURCES_ENABLED
            and settings.JOBS_SOURCE_REMOTEOK_ENABLED
            and criteria.params.remote_mode in {"remote", "any"}
        )
    ...
    def fetch(self, criteria: FetchCriteria, db: Session | None = None) -> list[Job]:
        ...
        for i, item in enumerate(payload):
            ...
            if not title or not apply_url:
                _skip("remoteok", slug or "-", i, "missing_required", item)
                continue
            tags_raw = item.get("tags")
            tags: list = tags_raw if isinstance(tags_raw, list) else []
            skills = [str(t).strip() for t in tags if str(t).strip()] or _skills(desc, criteria.profile)
...
class AdzunaSource:
    ...
    def fetch(self, criteria: FetchCriteria, db: Session | None = None) -> list[Job]:
        ...
        for i, item in enumerate(payload["results"]):
            if not isinstance(item, dict):
                _skip("adzuna", "-", i, "not_object", item)
                continue
            title, desc = str(item.get("title") or "").strip(), str(item.get("description") or "").strip()
            apply_url = str(item.get("redirect_url") or "").strip()
            if not title or not desc or not apply_url:
                _skip("adzuna", "-", i, "missing_required", item)
                continue
            ...
            out.append(
                _job(
                    ...
                    source_job_id=str(item.get("id") or apply_url),
                    title=title,
                    company=str(co.get("display_name") or "Unknown").strip(),
                    location=str(loc.get("display_name") or where).strip(),
                    description=desc,
                    apply_url=apply_url,
                    posted_at=parse_iso(item.get("created")),
                    skills=_skills(desc, criteria.profile),
                    salary_min=sal,
                )
            )
        return out
