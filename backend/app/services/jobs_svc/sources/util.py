...
from app.services.jobs_svc.filters import DATE_WINDOW_DAYS, infer_remote_mode
from app.services.jobs_svc.sources.base import FetchCriteria

logger = get_logger(__name__)
BOARD_TTL = timedelta(hours=6)
_ATS_CONFIG = Path(__file__).resolve().parents[5] / "configs" / "ats_companies.json"
...
_NON_US = frozenset(
    "japan tokyo china india bangalore uk london germany berlin france paris canada toronto sydney australia singapore korea seoul ireland dublin netherlands amsterdam brazil mexico israel emea apac".split()
)
...
def _board_row(db: Session, source: str, slug: str):
    ...
    return (
        db.query(JobCache)
        .filter(JobCache.workspace_id == wid, JobCache.source == f"{source}_board", JobCache.source_job_id == slug)
        .one_or_none()
    )
