...
def _near_dup(a: str, b: str) -> bool:
    if not a or not b or a == b:
        return a == b
    if a in b or b in a:
        return abs(len(a) - len(b)) <= max(6, int(0.25 * max(len(a), len(b))))
...
def dedupe_jsearch_queries(queries: list[str], *, max_n: int | None = None) -> list[str]:
    ...
    for q in queries:
        ...
        if not key or any(_near_dup(key, n) for n in norms):
            continue
        norms.append(key)
        out.append(cleaned)
        if len(out) >= cap:
...
def _extract_jsearch_items(payload: object) -> list[dict]:
    ...
    if isinstance(data, dict):
        for key in ("jobs", "results", "job_results", "items"):
            ...
            if key == "jobs" and isinstance(nested, list):
...
def _is_quota_response(status: int, body_text: str) -> bool:
    ...
    return status in {403, 402} and any(
        t in low for t in ("quota", "rate limit", "exceeded your monthly", "too many requests")
    )
...
def _with_cache_session(fn):
    ...
    try:
        ...
        logger.warning("jobs.jsearch_cache_failed", error=type(exc).__name__)
        return None
    finally:
        session.close()
...
def jsearch_get(params: dict[str, str], *, db=None) -> list[dict]:
    ...
    try:
        with httpx.Client(timeout=default_timeout()) as client:
            ...
            if _is_quota_response(response.status_code, preview):
        ...
        if _is_quota_response(status, preview):
            raise JSearchQuotaError() from exc
...
def build_jsearch_queries(title: str, location: str, skills: list[str] | None = None) -> list[str]:
    ...
    if "remote" not in loc.lower() and "remote" not in role.lower():
        add(f"{role} remote")
    if skill_bits:
...
def fetch_jsearch_raw(queries: list[str], *, base_params: dict[str, str], db=None) -> tuple[list[dict], int, int]:
    ...
    with ThreadPoolExecutor(max_workers=min(_JSEARCH_CONCURRENCY, max(1, len(capped) or 1))) as pool:
        for fut in as_completed([pool.submit(one, q) for q in capped]):
            try:
                ...
                failed += 1
                first_error = first_error or ServiceFailingError("Jobs API", "upstream request failed")
                continue
            for item in items:
    ...
    try:
        ...
        record_trace(
            operation="jsearch.search",
            status="ok" if merged or first_error is None else "error",
            input_tokens=requests_made,
            output_tokens=len(merged),
            error_type=type(first_error).__name__ if first_error and not merged else None,
            prompt_name="jsearch_requests_per_search",
            prompt_version=str(requests_made),
        )
