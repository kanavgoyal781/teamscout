...
def skill_requirement_score(
    ...
    for ut in unit_texts:
        if not phrase_in_text(requirement_text, ut):
            continue
...
def align_resume(
    ...
    for i, (req_emb, req_text, weight) in enumerate(zip(requirement_embs, requirement_texts, weights, strict=True)):
        ...
        if is_skill:
            ...
            # Prefer experience bullets over skill-list labels when both match.
        ...
        # Zero contribution (raw < floor OR raw == floor → rescaled 0) is always a miss.
