...
from app.services.job_filters import annotate_job, jsearch_params_from_search
...
from app.services.jsearch_client import fetch_jsearch_raw
...
class JSearchSource:
    ...
    def fetch(self, criteria: FetchCriteria, db: Session | None = None) -> list[Job]:
        ...
        for i, item in enumerate(raw):
            ...
            skills = item.get("job_required_skills") if isinstance(item.get("job_required_skills"), list) else []
            skills = [str(s).strip() for s in skills if str(s).strip()] or _skills(desc, criteria.profile)
...
class AdzunaSource:
    ...
    def fetch(self, criteria: FetchCriteria, db: Session | None = None) -> list[Job]:
        ...
        for i, item in enumerate(payload["results"]):
            ...
            co = item.get("company") if isinstance(item.get("company"), dict) else {}
