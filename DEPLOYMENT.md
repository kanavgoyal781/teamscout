...
> **Honesty:** This repo ships deploy **config + CI + demo-check**. A live
> public URL exists only after an operator runs the steps below with real
> account tokens and API keys. If `FLY_API_TOKEN` / `VERCEL_TOKEN` are not in
> GitHub Environment secrets, CI skips or blocks deploy; that is expected.
...
- Behind Fly’s proxy, rate-limit keys use the direct peer IP (see `ARCHITECTURE.md`). Fine for light demos; multi-tenant production may need trusted-hop keying later.
...
| Name | Value |
|---|---|
| `NEXT_PUBLIC_API_BASE` | `https://teamscout-api.fly.dev` (your Fly URL, no trailing slash) |
