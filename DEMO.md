...
**UI truth (do not improvise labels):** sidebar uses **Feature 1** / **Feature 2** / **About**; product copy says **hiring team**, not “Sumble”. Feature 2 cards show **Coverage** (MaxSim %) vs **Overall match** (ring); when a close-call tournament reorders the list, the **Ranked by close-call tournament** badge appears.
...
|---|---|---|---|
| 1 | Scope + offline gate (optional if CI green) | `make pipeline-offline` or at least `make check-scope` | exit 0 |
...
| 6 | Sumble credits remaining | Provider dashboard / ops traces / recent team call cost | Enough headroom for **one** find-team (~org + job-match or people path; UI estimates ~20–30 credit band before lookup) + optional **one** email reveal (preview shows cost, confirm spends). Prefer **skipping confirm reveal** if credits are tight. |
| 7 | Cost-ceiling headroom | `.env`: `LLM_DAILY_COST_CEILING_USD` (default 5), `SUMBLE_DAILY_CREDIT_CEILING` (default 1000) | Ceilings not already exhausted (exceed → HTTP 429 with `cost_ceiling_exceeded`). Avoid re-running full search loops in rehearsal right before the take. |
...
| **0:20–0:50** | Feature 1 | Drop zone **Drag & drop resume** → choose `samples/sample_resume.pdf` (or click browse). Click **Upload & parse**. Wait for parse toast / **Confirm profile** panel. | Upload hits `POST /resumes/upload`; LLM `complete_json` fills `ResumeProfile` (title, location, skills). Unconfigured LLM → typed 503, not mock data. |
| **0:50–1:10** | Confirm profile | Review title / location / skills chips. Click **Confirm profile**, then **Search jobs** (primary). Leave filters at defaults unless asked. | Search requires confirmed profile (`canSearch`); pipeline is multi-query fetch → dense+BM25 → RRF → batched LLM rerank → weighted fuse (defaults 0.38 LLM / 0.20 RRF / 0.12 skill / 0.12 exp / 0.10 req / 0.08 recency). |
...
| **1:40–2:15** | Hiring team | On the same card: **Find the team** → **Extract team from description** → wait for extraction card (team / department / likely hiring titles) → **Confirm & find hiring team**. When people appear, optionally **Reveal email — preview cost** only (say “confirm spends credits” and stop, or confirm once if checklist #6 allows). | Extract is LLM-on-JD; people lookup is Sumble (org + job-post match primary, people filter fallback). Email reveal is two-step; DB `email_reveals` avoids double-charge. |
| **2:15–2:45** | Feature 2 | Sidebar **Feature 2**. Panel **1. Resume library**: multi-select 2–3 PDFs (include `samples/sample_resume.pdf` plus 1–2 distinct others) → **Upload files or ZIP**. Panel **2. Paste a job description**: title optional, paste the JD below. Click **Find best resume for this job**. | MaxSim path: JD → atomic requirements → unit coverage; optional close-call tournament when scores are near the threshold. **Not** hybrid RRF. Button label is **Find best resume for this job**, not “Pick best resume”. |
| **2:45–3:00** | Comparison | Point at **Best match** card: **Coverage** % (pure MaxSim), **Overall match** ring, score bars (Coverage / LLM fit / Skill / Experience), alignment/evidence table. If present, call out **Ranked by close-call tournament** and the meta line **Close-call tournament: N pairwise comparison(s)**. Optional: **About** for funnel diagram if time left. | Eval suite `resume_pick` in `evals/history.jsonl` is the regression gate (`make eval` / `scripts/eval_resume_pick.py`). Coverage is not RRF; tournament only reorders a narrow band and logs cost to ops traces. |
...
| Feature 2 “Add resumes above first” / empty library | Empty library | Upload at least one PDF |
| Identical files don’t add rows | Content-hash dedup | Use distinct files |
| No tournament badge | Scores not close enough / tournament did not override | Still show **Coverage** vs **Overall match**; badge only when `overrode_coverage` is true |
